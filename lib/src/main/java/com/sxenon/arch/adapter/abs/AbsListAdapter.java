...
public abstract class AbsListAdapter<R> extends BaseAdapter implements IWosaiAdapter<R> {
     ...
     */
    public AbsListAdapter(IListViewModule<R> container, @NonNull AbsListItemViewTypeEntity[] itemViewTypeEntryArray) {
    ...
    public View getView(int position, View convertView, @NonNull ViewGroup parent) {
        AbsListViewHolder viewHolder = null;
        if (convertView == null) {
            AbsListItemViewTypeEntity itemViewTypeEntity = mItemViewTypeEntryArray[getItemViewType(position)];
            convertView = LayoutInflater.from(mContainer.getContext()).inflate(itemViewTypeEntity.getResourceId(), null);
            Class<? extends AbsListViewHolder> viewHolderClass = itemViewTypeEntity.getViewHolderClass();
            try {
                Constructor<? extends AbsListViewHolder> constructor = viewHolderClass.getConstructor(View.class, AbsListAdapter.class, Integer.class);
                viewHolder = constructor.newInstance(convertView, AbsListAdapter.this, position);
            } catch (Exception e) {
                e.printStackTrace();
            }
            convertView.setTag(viewHolder);
        } else {
            viewHolder = (AbsListViewHolder) convertView.getTag();
        }
